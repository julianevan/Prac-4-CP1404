...
for i in range(1, len(full_names)+1):
    lowercase_full_names[i-1] = (lowercase_full_names[i-1]).lower()
print(lowercase_full_names)
almost_numbers = ['0', '10', '21', '3', '-7', '88', '9']
# TODO: use a list comprehension to create a list of integers
# from the above list of strings
string_numbers=[]
for i in range(1,len(almost_numbers)+1):
    almost_numbers[i-1] = str(almost_numbers[i-1])
    string_numbers.append(almost_numbers[i-1])
print(string_numbers)
# TODO: use a list comprehension to create a list of only the numbers that are
# greater than 9 from the numbers (not strings) you just created
greater_than_nine_list = []
for i in range(1,len(almost_numbers)+1):
    if int(almost_numbers[i-1]) > 9:
        greater_than_nine_list.append(almost_numbers[i-1])
print(greater_than_nine_list)
